...
class CraftNPK(
    ...
    init {
        ...
        packetPlayOutPlayerInfoAdd = PacketPlayOutPlayerInfo(EnumPlayerInfoAction.ADD_PLAYER, nmsPlayer)
        packetPlayOutPlayerInfoRemove = PacketPlayOutPlayerInfo(EnumPlayerInfoAction.REMOVE_PLAYER, nmsPlayer)
        ...
        scoreboardTeam.a(EnumNameTagVisibility.NEVER)
    ...
    private fun getCollection(server: WorldServer): IntHashMap? {
        try {
            val field: Field = WORLD_CLASS.getDeclaredField("entitiesById")

            field.isAccessible = true

            return field[server] as IntHashMap
